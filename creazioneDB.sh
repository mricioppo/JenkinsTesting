...
zypper --non-interactive ar -f  https://download.opensuse.org/repositories/network:/ldap/15.4/network:ldap.repo
...
zypper --non-interactive install openldap2-devel
zypper --non-interactive install openssl-devel
zypper --non-interactive install mlocate 
...
zypper --non-interactive install 389-ds
zypper --non-interactive install zlib-devel
echo "Installo postgres 12.3 TDE"
...
cd ../..
cat << EOF > key_manager.sh
#!/bin/sh 
echo 882fb7c12e80280fd664c69d2d636913
EOF
chmod +x key_manager.sh 
mv key_manager.sh /home
mkdir /opt/db12tde
chown postgres /opt/db12tde
HERE
sudo su postgres <<HERE
/usr/local/pg12tde/bin/initdb -D /opt/db12tde -K /home/key_manager.sh
HERE
sudo su <<HERE
cd /opt/db12tde
...
encryption_key_command = '/home/key_manager.sh'
...
HERE
sudo su postgres << HERE
/usr/local/pg12tde/bin/pg_ctl -D /opt/db12tde start
/usr/local/pg12tde/bin/psql <<EOF
create database gecodb;
EOF
/usr/local/pg12tde/bin/psql -h $1 -d gecodb -U postgres -f /tmp/gecodb.sql
HERE